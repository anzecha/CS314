...
public int prim( Vertex s ) 
{
	...
	s.cost = 0;
	PriorityQueue<Vertex> fringe = new PriorityQueue<Vertex>(20, new Comparator<Vertex>() 
			{public int compare(Vertex i, Vertex j) {return (i.cost - j.cost); }});
	fringe.add(s);
	while ( ! fringe.isEmpty() ) 
	{
...
{
	Cons result = Cons.list(city);
//	String parcity = vertex(city).parent().contents;
	while(vertex(city).parent() != null)
	{
		result = Cons.append(Cons.list(vertex(city).parent().contents),result);
		city = vertex(city).parent().contents;
	}
