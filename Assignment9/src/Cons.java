...
//    public static boolean unaryp(Cons rh)
//    {
//    	if(rhs(rh) == null)
//    		return true;
//    	return false;
...
//    {
//    	if(stringp(lhs(e)) && lhs(e).equals(v))
//    		return e;
//    	if(stringp(rhs(e)) && rhs(e).equals(v))
//    		return list("=", rhs(e), lhs(e));
//    	if(!consp(rhs(e)))
//    		return null;
//    	if(unaryp((Cons) rhs(e)) || op((Cons)rhs(e)).equals("expt"))
//    		if(op((Cons) rhs(e)).equals("sqrt"))
//    			return solve(list("=", list(opposite((String) op((Cons)rhs(e))), lhs(e), new Integer(2)), lhs((Cons)rhs(e))), v);
//    		else if(op((Cons) rhs(e)).equals("-"))
//    			return solve(list("=",list("-", lhs(e)), lhs((Cons)rhs(e))),v);
//    		else
//    			return solve(list("=", list(opposite((String)op((Cons) rhs(e))), lhs(e)), lhs((Cons) rhs(e))), v);
//    	Cons sol = null;
//    	sol = solve(list("=", list(opposite((String) op((Cons)rhs(e))), lhs(e), rhs((Cons)rhs(e))), lhs((Cons)rhs(e))),v);
//    	if(sol != null)
//    		return sol;
//    	if(op((Cons)rhs(e)).equals("-") || op((Cons)rhs(e)).equals("/"))
//    		return solve(list("=", list(op((Cons)rhs(e)),lhs((Cons)rhs(e)),lhs(e)),rhs((Cons)rhs(e))),v);
//    	return solve(list("=", list(opposite((String) op((Cons) rhs(e))), lhs(e), lhs((Cons)rhs(e))), rhs((Cons)rhs(e))),v);
//    }
    public static Cons solve(Cons e, String v) 
    {
    	Cons try1;
    	...
    	else if(consp(rhs(e)))
    	{
    		while(copy != null)
    		{
    			if((trans = (Cons)transform((Cons)first(copy), e)) != null)
    			{
    				if((try1 = solve(trans, v)) != null)
    					return try1;
    				else
    					copy = rest(copy);
    			}
    ...
    public static int conshash(Object e) 
    {
    	...
    	if(consp(e))
    	{
    		...
    		return e.hashCode();
    	/*
    	 * hashCode(first(list)) * 17 ^ haschode(rest(list))*127
    	 * 
    	 *  
    	 */
    ...
    public static int totaltime(Cons action) 
    {
    	...
    	{
    		Cons copy = rest(action);
    		int sum = 0;
    		while(copy != null)
    		{
    			sum += totaltime((Cons)first((Cons)copy));
    			copy = rest(copy);
    		}
    		return sum;
    	}
    	...
    	if (op(action).equals("sync"))
    	{
    		...
    		while(copy != null)
    		{
    			int temp = Math.max(max, totaltime((Cons)(first((Cons)copy))));
    			if(temp > max)
    			{
    				max = temp;
    			}
    ...
    public static void execute(PriorityQueue pq, Cons act, int time) {
        ...
        if ( command.equals("kaboom") ) {
        	int tame = (int)(Integer)second(act);
        	int tfour = tame/4;
        	int ttwo = tame/2;
        	addevent(pq, list("seq", list("rest", tfour), list("boom", tfour), list("boom", ttwo)), time);
            return; }
        if ( command.equals("tambourine") ) {
        	double duration = (int)(Integer)second(act) * random.nextDouble();
        	if(duration >= 2)
        	{
        		int tame = time + (int)(Integer)second(act) - (int)duration;
        		addevent(pq, list("tambourine", (int)duration), tame);
        	}
        	emit(11, time, 2);
            return; }
        if ( command.equals("piano") ) {
        	Cons resact = rest(act);
        	int voice = (int)(Integer)first(resact);
        	String note = (String)second(resact);
        	int d = (int)(Integer)third(resact);
        ...
        if ( command.equals("seq") ) {
        	...
        	while(copy != null)
        	{
        		Cons firstact = (Cons)first(copy);
        		addevent(pq, (Cons)first(copy), time);
...
//        		Cons firstact = (Cons)first(copy);
        ...
        if ( command.equals("repeat") ) {
        	Cons theact = (Cons) third(act);
        	int times = (Integer) second(act);
        	addevent(pq, theact, time);
        	if(times > 1)
        	{
        		time += totaltime(theact);
        		addevent(pq, list("repeat", times-1, theact), time);
        	}
    ...
    {
    	int tottime = totaltime(melody);
    	int totfour = tottime / 4;
    	int tottwo = tottime / 2;
    	...
    	return list("sync",list("seq", list("rest", 0), firstvoice), list("seq", list("rest", totfour), secondvoice), list("seq", list("rest", tottwo), thirdvoice));
...
//	Cons melody = (Cons) reader("(sync (seq (boom 2) (rest 4) (kaboom 4) (rest 6) (boom 2) (rest 4) (kaboom 4) (boom 2) (rest 0)) (boom 3) (repeat 16 (seq (hat 1) (rest 1))))");
