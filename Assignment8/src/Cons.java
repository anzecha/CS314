...
{
	//If the desired v has a value defined in vals return that value
	if(assoc(v, vals) != null)
	{
		Cons numb = rest(assoc(v, vals));
		return (Double)first(numb);
	}
	//get the vars of vals
	Cons temp_vals = vals;
	Cons vars_of_known_values = null;
	while(temp_vals != null)
	{
		vars_of_known_values = append(list(first((Cons)first(temp_vals))), vars_of_known_values);
		temp_vals = rest(temp_vals);
	}
	Cons equation = (Cons) first(eqns);
	Cons variables_equation = vars(equation);
	//check if the equation has only one unknown
	Cons difference = setDifference(variables_equation, vars_of_known_values);
	if(length(difference) == 1)
	{
		String unknown_var = (String)first(difference);
		Cons new_eqn = solve(equation, unknown_var);
		Double value_var = eval(rhs(new_eqn), vals);
		Cons new_binding = list(unknown_var, value_var);
		vals = append(list(new_binding), vals);
		return solveqns(eqns, vals, v);
	}
	else if(length(difference) != 1 && eqns != null)
	{
    ...
    public static Cons derivpats = readlist( list(
       ...
       "( (deriv (- ?u ?v) ?x)  (- (deriv ?u ?x) (deriv ?v ?x)))",
       "( (deriv (- ?v) ?x)  (- (deriv ?v ?x)))",
       "( (deriv (* ?u ?v) ?x)  (+ (* ?u (deriv ?u ?x)) (* ?v (deriv ?v ?x))))",
    ...
    public static Cons javapats = readlist( list(
       ...
       "( (>= ?x ?y)  (zlparen ?x zspace >= zspace ?y zrparen))",
       "( (<= ?x ?y)  (zlparen ?x zspace <= zspace ?y zrparen))",
       "( (expt ?x ?y)  (zlparen Math.pow zlparen ?x zspace , zspace ?y zrparen zrparen))",
       "( (incf ?x) (znothing ?x znothing ++) )",
       "( (or ?x ?y)  (zlparen ?x zspace || zspace ?y zrparen))",
       "( (and ?x ?y)  (zlparen ?x zspace && zspace ?y zrparen))",
       "( (not ?x)  (znothing ! znothing ?x))",
       "((return ?x) (return zspace ?x ;))",       	
       "( (if ?test ?then ?else) (if zspace zlparen zspace ?test zspace zrparen zspace { ztab zreturn ?then zuntab zreturn } zreturn else zspace { ztab zreturn ?else zuntab zreturn }))",
