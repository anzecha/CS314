        ...
        final Mapper map2 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                 ...
                 if(!value.equals(""))
                 {
                	 ...
                	 for(String p = value; p.length() != 0; p=p.substring(1))
                	 { 
                		 char s = p.charAt(0);
...
//                			 System.out.println("   map2 key = " + key + "  val = " + p);
                			 mr.collect_map(Character.toString(s), list("1"));
                		 }
...
//                 	 WHAT ABOUT THIS???
                	 ...
                	 while (token.hasMoreTokens()){
                		 mr.collect_map(value = token.nextToken(), list("1"));
        ...
        final Mapper map4 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                while(value.contains(grepkey))
                {
                	mr.collect_map(grepkey, list(line));
                	value = value.substring(value.indexOf(grepkey) + 1);
                }
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	for ( Cons lst = value; lst != null; lst = rest(lst) )
            	{
            		result = append(result, list(Integer.decode((String) first((Cons)first(lst)))));
            	}
                mr.collect_reduce(key, result);
            } };
...
//                	 Make it case-insensitive
               	 ...
               	 while (token.hasMoreTokens()){
               		 mr.collect_map((value = token.nextToken()), list(key));
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	movie = value.substring(0, firstcomma);
            	//System.out.println(movie);
            	value = value.substring(firstcomma + 1);
            	int secondcomma = value.indexOf(',');
            	rating = value.substring(secondcomma + 1, secondcomma + 2);
            	//System.out.println(rating);
            	mr.collect_map(movie, list(rating));
            } };
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                result = (sum/count);
                mr.collect_reduce(key, list(result, (int)count));
            } };
